...
export default function LiveTrackingView({
  ...
  if (!order) {
    return (
      <div
        id="no-order-tracker"
        className="bg-slate-50 border border-slate-200/60 rounded-2xl p-10 flex flex-col items-center justify-center text-center h-[525px]"
      >
        ...
        <p className="text-xs text-slate-500 max-w-sm mt-1">
          Type can you request or confirm an order in the Telegram bot chat
          simulator to start active real-time tracking.
        </p>
      </div>
    );
  }

  const currentCoords = getScooterCoords(
    order.status === "preparing" ? 0 : (order.progress - 30) * (100 / 70),
  );
  ...
  return (
    <div
      ...
      className="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-sm h-[620px] flex flex-col"
    >
      {/* Header Info */}
      ...
      <div className="grid grid-cols-4 border-b border-slate-100 bg-slate-50/50 p-3 text-center text-xs">
        <div
          className={`p-1 border-r border-slate-100 flex flex-col items-center ${order.progress >= 0 ? "text-[#E0560B] font-semibold" : "text-slate-400"}`}
        >
          <div
            className={`w-6 h-6 rounded-full flex items-center justify-center text-[10px] mb-1 ${order.progress >= 5 ? "bg-[#E0560B] text-white" : "bg-slate-200 text-slate-600"}`}
          >
            1
          </div>
          <span>Received</span>
        ...
        <div
          className={`p-1 border-r border-slate-100 flex flex-col items-center ${order.progress >= 10 ? "text-[#E0560B] font-semibold" : "text-slate-400"}`}
        >
          <div
            className={`w-6 h-6 rounded-full flex items-center justify-center text-[10px] mb-1 ${order.progress >= 30 ? "bg-[#E0560B] text-white" : "bg-slate-200 text-slate-600"}`}
          >
        ...
        <div
          className={`p-1 border-r border-slate-100 flex flex-col items-center ${order.progress >= 30 ? "text-[#E0560B] font-semibold" : "text-slate-400"}`}
        >
          <div
            className={`w-6 h-6 rounded-full flex items-center justify-center text-[10px] mb-1 ${order.progress >= 85 ? "bg-[#E0560B] text-white" : "bg-slate-200 text-slate-600"}`}
          >
        ...
        <div
          className={`p-1 flex flex-col items-center ${order.progress === 100 ? "text-green-600 font-semibold" : "text-slate-400"}`}
        >
          <div
            className={`w-6 h-6 rounded-full flex items-center justify-center text-[10px] mb-1 ${order.progress === 100 ? "bg-green-600 text-white" : "bg-slate-200 text-slate-600"}`}
          >
            4
          </div>
          <span>At Door</span>
      ...
      <div className="relative flex-1 bg-sky-200/20 overflow-hidden">
        ...
        <svg
          viewBox="0 0 740 400"
          className="w-full h-full bg-[#f4ebd0] opacity-90 transition-all select-none"
        >
          {/* Map Grid Grid roads */}
          ...
          <text
            ...
            y="100"
            fill="#5c8246"
            className="text-[10px] font-semibold tracking-wider italic fill-opacity-65"
            textAnchor="middle"
          >
          ...
          {/* Sinuous River blue */}
          <path
            d="M -40,350 Q 180,310 320,380 T 700,280 T 800,320"
            fill="none"
            stroke="#add8e6"
            strokeWidth="26"
            strokeLinecap="round"
          ...
          {/* Streets Grids / Roads */}
          <path
            d="M 40,80 H 700 M 40,140 H 700 M 40,220 H 700 M 40,320 H 700 M 120,40 V 360 M 340,40 V 360 M 500,40 V 360 M 620,40 V 360"
            fill="none"
            stroke="#fafafa"
            strokeWidth="14"
            strokeLinecap="round"
          ...
          <text
            ...
            y="130"
            fill="#a49980"
            className="text-[9px] font-mono tracking-tight font-medium"
          >
            Main Broadway St.
          </text>
          <text
            ...
            y="300"
            fill="#a49980"
            className="text-[9px] font-mono tracking-tight font-medium font-semibold uppercase origin-center rotate-90"
          >
            River Parkway
          </text>
          <text
          ...
          <circle
            ...
            opacity="0.15"
          />
          <circle cx="120" cy="140" r="6" fill="#E0560B" />
          <g transform="translate(100, 105)">
            <rect width="64" height="18" rx="4" fill="#E0560B" />
            <text
              x="32"
              y="12"
              fill="#ffffff"
              className="text-[8px] font-sans font-bold text-center"
              textAnchor="middle"
            >
              OUR KITCHEN
            </text>
          ...
          <path
            d="M 120,140 H 340 V 220 H 500 V 320 H 620"
            fill="none"
            stroke="#E0560B"
            strokeWidth="4"
            strokeLinecap="round"
          ...
          {order.status !== "preparing" && (
            <g
              transform={`translate(${currentCoords.x - 14}, ${currentCoords.y - 14})`}
            >
              {/* Pulsing indicator */}
              <circle
                cx="14"
              ...
              <g transform="translate(6, 6) scale(0.6)">
                <path
                  d="M21 16H18V12H21V16ZM14.99 12H11.99V16H14.99V12ZM14.99 18H11.99V22H14.99V18ZM21 18H18V22H21V18ZM2.99 12H5.99V16H2.99V12ZM8.99 12H5.99V16H8.99V12ZM8.99 18H5.99V22H8.99V18ZM2.99 18H5.99V22H2.99V18ZM11 5V8.5L8.5 11H3.5C2.12 11 1 9.88 1 8.5C1 7.12 2.12 6 3.5 6H6.5L8.5 4H10.5V5ZM13 7H14.5V11C14.5 12.1 13.6 13 12.5 13H5C4.45 13 4 12.55 4 12C4 11.45 4.45 11 5 11H12V8C12.55 8 13 7.55 13 7Z"
                  fill="#ffffff"
                />
              </g>
              {/* Miniature Arrow */}
              <polygon
                points="14,2 18,8 10,8"
                fill="#e11d48"
                transform={`rotate(${currentCoords.rot}, 14, 14)`}
              />
            </g>
          )}
        </svg>
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-sm font-semibold text-slate-800 truncate">
              {order.status === "payment_pending"
                ? "Waiting for administrative payment verification..."
                : order.status === "pending"
                  ? "Payment verified! Awaiting driver assignment..."
                  : order.status === "preparing"
                    ? "Chef is handcrafting your meal inside ready-kitchen..."
                    : order.status === "driving"
                      ? `Driver ${order.driverName || "Rider"} picked up and navigating Broad Street...`
                      : order.status === "completed"
                        ? "Dispatched scooter arrived outside your door! Enjoy!"
        ...
        <div className="flex items-center gap-3 bg-white p-3 rounded-xl border border-slate-150">
          ...
          <div className="min-w-0 flex-1">
            ...
            <span className="text-xs text-slate-500">
              Mailed instructions: Ring doorbell or call.
            </span>
          </div>
        </div>
      </div>
